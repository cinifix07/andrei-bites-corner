...
const navigation = [
  ...
  ['analytics', 'Analytics'],
  ['archive', 'Archives'],
  ['manage_accounts', 'User Management'],
...
const inventoryCategoryFilters = ['ALL', 'BURGERS', 'SNACKS', 'PIZZA', 'DRINKS', 'SHAKES', 'FRIES']
...
export default function Admin({ onSignOut }) {
  ...
  const [activeNav, setActiveNav] = useState(() => {
    ...
    return adminViews.has(savedView) ? savedView : 'Dashboard'
  })
  const [range, setRange] = useState('Monthly')
  const [isMenuOpen, setIsMenuOpen] = useState(false)
  const [isArchivesOpen, setIsArchivesOpen] = useState(() => window.localStorage.getItem(adminArchivesStorageKey) === 'true')
  const [inventoryAlertsPage, setInventoryAlertsPage] = useState(1)

  useEffect(() => {
    window.localStorage.setItem(adminViewStorageKey, activeNav)
  }, [activeNav])

  useEffect(() => {
    window.localStorage.setItem(adminArchivesStorageKey, String(isArchivesOpen))
  }, [isArchivesOpen])
  ...
  const stats = dashboardStats ?? { netProfit: 0, profitPercentage: 0, dailyRevenue: 0, weeklyRevenue: 0, monthlyRevenueTotal: 0, totalRevenue: 0, totalInvestment: 0, yearlyRevenueTotal: 0, dailyRevenueBuckets: [0, 0, 0, 0, 0, 0, 0], monthlyRevenue: Array.from({ length: 12 }, () => 0), yearlyRevenue: Array.from({ length: 5 }, () => 0), yearlyLabels: [] }
  ...
  return (
    <div className="admin-shell">
      ...
      <main className="admin-main">
        {activeNav === 'Inventory' ? <InventoryView /> : activeNav === 'Profit' ? <ProfitView /> : activeNav === 'Analytics' ? <AnalyticsView /> : activeNav === 'User Management' ? <UserManagementView /> : (
          <>
        ...
        <section className="metrics-grid" aria-label="Key performance metrics">
          <MetricCard label="Net Profit" value={pesoFormatter.format(stats.netProfit)} badge={`${stats.profitPercentage >= 0 ? '+' : ''}${stats.profitPercentage.toFixed(1)}%`} badgeTone={stats.profitPercentage >= 0 ? 'positive' : 'negative'}>
            ...
            <p className="metric-detail">Total from Profit records</p>
          </MetricCard>
...
function InventoryView() {
  ...
  const filteredProducts = products.filter((product) => {
    const matchesStockStatus = selectedStockStatus === 'all' || getStockStatus(product.stock).tone === selectedStockStatus
    const matchesCategory = selectedCategory === 'ALL' || product.category?.toUpperCase() === selectedCategory
    const matchesSearch = !normalizedInventorySearch ||
      product.name.toLowerCase().includes(normalizedInventorySearch) ||
      product.category?.toLowerCase().includes(normalizedInventorySearch)

    return matchesStockStatus && matchesCategory && matchesSearch
  })
  ...
  const updateInventorySearch = (event) => {
  ...
  const saveProduct = async (event) => {
    ...
    try {
      if (photo instanceof File && photo.size > 0) {
        ...
        product.imageStorageId = editingProduct.imageStorageId
      }
  ...
  return (
    <section className="inventory-view">
      ...
      <section className="inventory-search-panel" aria-label="Search and filter inventory">
        <label className="inventory-search-field">
          <span className="material-symbols-outlined" aria-hidden="true">search</span>
          <input
            aria-label="Search products"
            onChange={updateInventorySearch}
        ...
        <div className="inventory-menu-categories" aria-label="Filter by product category">
      ...
      <div className="admin-card inventory-table-card">
        <table className="inventory-table">
          <thead>
            <tr><th>No.</th><th>Photo</th><th>Product Name</th><th>Category</th><th>No. of Stock</th><th>Price</th><th>Status</th><th>Date &amp; Time Added</th><th>Stock Updated</th><th>Actions</th></tr>
          </thead>
          <tbody>
            {visibleProducts.map((product, index) => (
              <tr key={product._id}>
                ...
                <td data-label="Photo">
                  <span className="inventory-product-photo">
                    {product.imageUrl ? <img src={product.imageUrl} alt={product.name} /> : <span className="material-symbols-outlined" aria-hidden="true">image</span>}
                  </span>
                </td>
        ...
        {filteredProducts.length === 0 ? <p className="inventory-empty">{products.length === 0 ? 'No products yet. Add your first product to get started.' : 'No products match your current filters.'}</p> : null}
      </div>
        ...
        <div className="product-modal-backdrop" role="presentation" onMouseDown={closeProductModal}>
          <section className="product-modal" role="dialog" aria-modal="true" aria-labelledby="product-modal-title" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <form key={editingProduct?._id ?? 'new'} onSubmit={saveProduct}>
              ...
              <label>Category
                <select name="category" defaultValue={editingProduct?.category ?? 'BURGERS'} required>
                  {inventoryCategoryFilters.filter((category) => category !== 'ALL').map((category) => (
                    <option key={category} value={category}>{category}</option>
                  ))}
                </select>
              </label>
              <label className="product-photo-field">Product Photo
                {editingProduct?.imageUrl ? <img className="product-photo-preview" src={editingProduct.imageUrl} alt={editingProduct.name} /> : null}
                <input name="photo" type="file" accept="image/*" />
                <small>{editingProduct?.imageUrl ? 'Upload a new image to replace the current photo.' : 'Upload a product image for menu and cashier displays.'}</small>
              </label>
...
function UserManagementView() {
  ...
  const saveUser = async (event) => {
    ...
    try {
      if (editingUser) {
        await updateUser({ id: editingUser._id, ...user })
      } else {
  ...
  const deleteUser = async () => {
    ...
    try {
      await removeUser({ id: pendingDeleteUser._id })
  ...
  return (
    <section className="users-view">
      <header className="inventory-heading">
        <div>
          <p className="inventory-eyebrow">ACCESS CONTROL</p>
          <h2>User Management</h2>
          <p>Manage administrator and customer user accounts.</p>
        </div>
        <button className="add-product-button" type="button" onClick={() => openUserModal()}>
          <span className="material-symbols-outlined" aria-hidden="true">person_add</span>
          Add User
        </button>
      </header>

      <div className="admin-card inventory-table-card users-table-card">
        <table className="inventory-table users-table">
          <thead>
            <tr><th>No.</th><th>Name</th><th>Role</th><th>Username</th><th>Password</th><th>Date Added</th><th>Actions</th></tr>
          </thead>
          <tbody>
            {users.map((user, index) => (
              <tr key={user._id}>
                ...
                <td data-label="Role"><span className={`user-role-badge ${user.role}`}>{user.role}</span></td>
                <td data-label="Username">{user.username}</td>
                <td data-label="Password"><span className="user-password-mask">••••••••</span></td>
                <td className="product-created" data-label="Date Added"><time dateTime={new Date(user._creationTime).toISOString()}>{productDateFormatter.format(user._creationTime)}</time></td>
                <td data-label="Actions">
                  <div className="inventory-actions">
                    <button type="button" aria-label={`Edit ${user.name}`} onClick={() => setPendingEditUser(user)}>
                      <span className="material-symbols-outlined" aria-hidden="true">edit</span>
                    </button>
                    <button className="delete" type="button" aria-label={`Delete ${user.name}`} onClick={() => {
                      setDeleteError('')
        ...
        {users.length === 0 ? <p className="inventory-empty">No users yet. Add your first user to get started.</p> : null}
      </div>

      {pendingEditUser ? (
        <div className="product-modal-backdrop" role="presentation" onMouseDown={() => setPendingEditUser(null)}>
          <section className="product-modal product-confirm-modal" role="alertdialog" aria-modal="true" aria-labelledby="user-edit-confirm-title" aria-describedby="user-edit-confirm-description" onMouseDown={(event) => event.stopPropagation()}>
            <div className="product-confirm-icon" aria-hidden="true"><span className="material-symbols-outlined">edit</span></div>
            <p className="product-success-eyebrow">CONFIRM EDIT</p>
            <h3 id="user-edit-confirm-title">Edit this user?</h3>
            <p id="user-edit-confirm-description">You are about to update <strong>{pendingEditUser.name}</strong>.</p>
            <div className="product-modal-actions">
              <button className="cancel" type="button" onClick={() => setPendingEditUser(null)}>Cancel</button>
              <button className="save" type="button" onClick={confirmUserEdit}>Yes, Edit User</button>
            </div>
          </section>
        </div>
      ) : null}
      ...
      {pendingDeleteUser ? (
        <div className="product-modal-backdrop" role="presentation" onMouseDown={() => !isDeleting && setPendingDeleteUser(null)}>
          <section className="product-modal product-confirm-modal product-delete-modal" role="alertdialog" aria-modal="true" aria-labelledby="user-delete-confirm-title" aria-describedby="user-delete-confirm-description" onMouseDown={(event) => event.stopPropagation()}>
            <div className="product-delete-icon" aria-hidden="true"><span className="material-symbols-outlined">delete</span></div>
            <p className="product-delete-eyebrow">DELETE USER</p>
            <h3 id="user-delete-confirm-title">Delete this user?</h3>
            <p id="user-delete-confirm-description"><strong>{pendingDeleteUser.name}</strong> will be permanently removed from user management.</p>
            {deleteError ? <p className="product-form-error" role="alert">{deleteError}</p> : null}
            <div className="product-modal-actions">
              <button className="cancel" type="button" disabled={isDeleting} onClick={() => setPendingDeleteUser(null)}>Cancel</button>
              <button className="delete-confirm" type="button" disabled={isDeleting} onClick={deleteUser}>{isDeleting ? 'Deleting…' : 'Yes, Delete User'}</button>
            </div>
          </section>
        </div>
      ) : null}
      ...
      {isModalOpen ? (
        <div className="product-modal-backdrop" role="presentation" onMouseDown={closeUserModal}>
          <section className="product-modal" role="dialog" aria-modal="true" aria-labelledby="user-modal-title" onMouseDown={(event) => event.stopPropagation()}>
            <div className="product-modal-header">
              <div><p>{editingUser ? 'UPDATE USER' : 'NEW USER'}</p><h3 id="user-modal-title">{editingUser ? 'Edit User' : 'Add User'}</h3></div>
              <button type="button" disabled={isSaving} aria-label="Close user form" onClick={closeUserModal}><span className="material-symbols-outlined" aria-hidden="true">close</span></button>
            </div>
            <form key={editingUser?._id ?? 'new-user'} onSubmit={saveUser}>
              <label>Name<input name="name" type="text" defaultValue={editingUser?.name ?? ''} placeholder="e.g. Andrei" required /></label>
              <div className="product-form-row">
                <label>Role
                ...
                <label>Username<input name="username" type="text" defaultValue={editingUser?.username ?? ''} placeholder="username" required /></label>
              </div>
              <label>Password<input name="password" type="text" defaultValue={editingUser?.password ?? ''} placeholder="password" required /></label>
              {formError ? <p className="product-form-error" role="alert">{formError}</p> : null}
              <div className="product-modal-actions">
                <button className="cancel" type="button" disabled={isSaving} onClick={closeUserModal}>Cancel</button>
                <button className="save" type="submit" disabled={isSaving}>{isSaving ? 'Saving…' : editingUser ? 'Save Changes' : 'Add User'}</button>
              </div>
            </form>
          </section>
        </div>
